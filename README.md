...
-----------------------------------------------------------  
The criteria stat code format (probably there are other formats too which i have not looked into)
  1) **c_code_paramX_paramY_statEvent_criteriaType**
  2) **c_paramX_paramY_statEvent_criteriaType**
-----------------------------------------------------------  

**paramX** and **paramY** can be empty string, which means many statcodes will have "__" in it.

## Stat Events

By looking at stat codes (for example "c_aku__shw_g") it is unclear what stat event (in this case "shw") means. I took a look at the battlelog apk awards json file (my interest in bf4 stats research appeared when i stumbled upon this file). I understood that these short names are stat events (for example "kwa" => StatEvent_KillWith_X_As_Y), so i looked further to get full stat event list. I searched for these stat events in bf4 ebx files (https://github.com/GreyDynamics/BF4_EBX) and found a place where stat event enum is used (https://github.com/GreyDynamics/BF4_EBX/blob/4e19b479df7294c70744fedf762575bdef9c6db8/Persistence/SpamSettings_offset_.txt). In order to get full stat event list, i needed to dump that particular ebx file, so i did and got the full stat event list (from server version R58). In order to get this stat event list usable i wrote this stat event enum in C#. You can see it down below. Comments - some descriptions i found for stat events while looking at bf4 ebx files.
...
# Criteria stat code examples

Now with this knowledge here are some examples  

1) **c_aek971__sfw_g**  
     - "c" - criteria  
     - "aek971" - paramX  
     - "" - paramY  
     - "sfw" - StatEvent_ShotsFired_With_X  
     - "g" - CriteriaType_GLOBAL_AllTimeTotal  
2) **c_kany_ak12_hsh_g**  
     - "c" - criteria  
     - "kany" - paramX  
     - "ak12" - paramY  
     - "hsh" - StatEvent_Headshot_X_With_Y
     - "g" - CriteriaType_GLOBAL_AllTimeTotal  
3) **c___ccp_g**
     - "c" - criteria  
     - "" - paramX  
     - "" - paramY  
     - "ccp" - StatEvent_CaptureCapturePoint
     - "g" - CriteriaType_GLOBAL_AllTimeTotal  
4) **c_as22_r25mv__ga_g**
     - "c" - criteria  
     - "as22" - code
     - "r25mv" - paramX  
     - "" - paramY  
     - "ga" - StatEvent_GainAward_X
     - "g" - CriteriaType_GLOBAL_AllTimeTotal  
